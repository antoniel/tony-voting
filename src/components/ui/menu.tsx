...
function MenuPopup({
  ...
  return (
    <MenuPrimitive.Portal>
      <MenuPrimitive.Positioner data-slot="menu-positioner" className="z-50" sideOffset={sideOffset} align={align} alignOffset={alignOffset}>
        <span className="relative flex origin-(--transform-origin) rounded-lg border bg-popover bg-clip-padding shadow-lg transition-[scale,opacity] before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-lg)-1px)] before:shadow-[0_1px_--theme(--color-black/4%)] has-data-starting-style:scale-98 has-data-starting-style:opacity-0 dark:bg-clip-border dark:before:shadow-[0_-1px_--theme(--color-white/8%)]">
          <MenuPrimitive.Popup data-slot="menu-popup" className={cn('max-h-(--available-height) min-w-32 overflow-y-auto p-1', className)} {...props} />
        </span>
...
function MenuShortcut({ className, ...props }: React.ComponentProps<'span'>) {
  return <span data-slot="menu-shortcut" className={cn('ms-auto text-xs tracking-widest text-muted-foreground/64', className)} {...props} />
}
...
function MenuSubPopup({
  ...
  return <MenuPopup className={className} sideOffset={sideOffset} align={align} alignOffset={alignOffset} data-slot="menu-sub-content" {...props} />
}
