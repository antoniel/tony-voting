...
function MeterTrack({ className, ...props }: MeterPrimitive.Track.Props) {
  return <MeterPrimitive.Track data-slot="meter-track" className={cn('block h-2 w-full overflow-hidden bg-input', className)} {...props} />
}

function MeterIndicator({ className, ...props }: MeterPrimitive.Indicator.Props) {
  return <MeterPrimitive.Indicator data-slot="meter-indicator" className={cn('bg-primary transition-all duration-500', className)} {...props} />
}

function MeterValue({ className, ...props }: MeterPrimitive.Value.Props) {
  return <MeterPrimitive.Value data-slot="meter-value" className={cn('text-sm tabular-nums', className)} {...props} />
}
